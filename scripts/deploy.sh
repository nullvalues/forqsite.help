...
#     or any container/service restart.
#
# Usage:
...
echo "backups   index.html.bak-${STAMP}, gap-handoff.html.bak-${STAMP}"
echo "target    the configured per-site directory, via the configured ssh alias"
echo "next      run the drift check to confirm the bytes a request returns (INFRA-007)"
